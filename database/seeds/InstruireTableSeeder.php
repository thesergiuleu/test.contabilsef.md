...
use App\Http\Controllers\Admin\InstruireController;
...
class InstruireTableSeeder extends Seeder
{
    /**
    ...
    {
        //Data Type
        $dataType = $this->dataType('slug', 'instruire');
        if (!$dataType->exists) {
            $dataType->fill([
                'name'                  => 'instruire',
                'display_name_singular' => __('instruire'),
                'display_name_plural'   => __('instruire'),
                'icon'                  => 'voyager-certificate',
                'model_name'            => Post::class,
                'controller'            => InstruireController::class,
        ...
        //Data Rows
        $postDataType = DataType::where('slug', 'instruire')->firstOrFail();
        ...
        $dataRow = $this->dataRow($postDataType, 'privacy');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'checkbox',
        ...
        $dataRow = $this->dataRow($postDataType, 'event_date');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
        ...
        $dataRow = $this->dataRow($postDataType, 'external_author');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'timestamp',
        ...
        $dataRow = $this->dataRow($postDataType, 'is_own');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'checkbox',
                'display_name' => __('Seminar organizat de ContabilSef'),
                'required'     => 0,
                'browse'       => 0,
                'read'         => 1,
                'edit'         => 1,
